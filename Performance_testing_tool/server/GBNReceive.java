...
public class GBNReceive extends Thread {
	...
	public GBNReceive(DatagramSocket socket, int port, String sno,int sport,InetAddress sip) throws UnknownHostException {
		this.socket = socket;
		this.port = port;
		this.sno = sno;
		this.sport = sport;
	...
	// part of local test
//	public GBNReceive(DatagramSocket socket, int port) {
//		this.socket = socket;
//		this.port = port;
//	}
		...
		while (!Thread.currentThread().isInterrupted()) {
			while (currentSeqNum <= 13) {
				...
				try {
					...
					System.out.println("wati from client");
					...
					// String fullData = new String(recvPacket.getData());
					...
					// System.out.println("test seq from client : " +
					// fullData.charAt(1));
					...
					if (compSeqNum != currentSeqNum) {
						...
						global.gbnStuInfo.replace(sno,
								new GBNResult(sno, sname, sip, sport, port, date, data, nak, afterNak, wSize, nakNum2));
					}
					 ...
					 if (checkNum == 7) {
						if (compSeqNum == currentSeqNum) {
							...
							global.gbnStuInfo.replace(sno, new GBNResult(sno, sname, sip, sport, port, date, data, nak,
									afterNak, wSize, nakNum2));
